...
//hace el llamado a la API Increase
const getApiInfo = async () => {
  ...
  //inicio array para guardar toda la info
  let clientes = [];
  ...
  //parseo todos los registros tipo1
  ...
  //parseo todos los registros tipo 2
  function parseRegistro2(linea, cliente) {
    //instancio objeto tipo registro 2
    let registro_2 = {
      idTransaccion: "",
      monto: "",
      tipo: "",
    };
    //separo cada dato a analizar
    ...
    registro_2.tipo =
      linea.substring(linea.length - 1, linea.length) === "1"
        ? "Aprobado"
        : "Rechazado";
  //mando al array de transacciones
    cliente.registro_2.push(registro_2);
    
  }

  //parseo todos los registros tipo 3
  function parseRegistro3(linea, cliente) {
    //separo datos a analizar
  ...
  //declaro array de errores de llamada  a la API Increase
  let apiError = false;
  let apiErrorids = [];
  
  //hago llamada al endpoint de cada cliente para obtener info de cada uno.
  async function datosClientes(cliente) {
    ...
    try {
      ...
      cliente.info = clientInfo;
        ...
        //en caso de que haya error, los meto en la db, porque puedo manipularlo, y buscarlo despues de ultima
      apiError = true;
      cliente.info = {
        id: id,
  ...
  async function guardarEnDb(cliente) {
    try {
      ...
      //cargo db con info de pagos de cada cliente
      await Pagos.create({
      ...
      //cargo db con las transacacciones.
  ...
  //esta funcion une todo, y ejecuta 
  async function executeGetApiInfo() {
    try {
       console.log('me ejecute')
      let file = await getApiInfo();
      //divido el archivo txt en lineas
      ...
      for (let i = 0; i < porLinea.length; i++) {
        ...
        if (tipo === "1") {
          ...
          //vuelvo a instanciar, limpiando el objeto para volver a ser utilizado
          cliente = {
            info: {},
            registro_1: {
              idPago: "",
